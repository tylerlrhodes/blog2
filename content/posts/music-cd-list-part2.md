...
I am going to make one change from how the code is presented from the
last post.  This post is going to rely more upon the Github repo to
review the code changes instead of long code blocks embedded in the
post.  I'll then demonstrate with a shorter code snippets the ideas
...
Github repo at the appropriate tags.
...
an approache for the overall structure without the implementation.

One possibility would be to require the collection passed in to also
be a `MutableSequence`, which is a sequence which can be updated and
derives from the `collections.abc` abstract base clase.
...
also select the key, and say whether to reverse the sort or not.
...
One option with the `ListStore` at this point would be to forget about
making it conform to `MutableSequence`, it's already taking a
collection to use for storage that does, and don't necessarily need to
...
The `LinkedList` and `ListStore` classes as they stand are not
thread safe, and certainly not protected against deadlocks or race
conditions, or doing anything to gaurantee data consistency.

If this wasn't an example application, I may not do anything.  But
it's a demonstration web application, and it would be nice to make it
...
to simple lock access to the list.  This is what I did from the Flask
web application, simply putting a lock around API calls which
interacted with the list.

This puts the onus on the user of `ListStore` as oppossed to the
...
It's now possible to swap out containers for the `ListStorage` without
